...
<html>
    ...
    <meta name="description" content=" شبکه اجتماعی اینتک پست یک شبکه اجتماعی مبتنی بر متن است که در آن می توان پست هایی تا حداکثر هزار کاراکتر و به همراه یک فایل صوتی، تصویری یت ویدیویی در هشت دسته بندی مختلف گذاشت.">
    <link rel="canonical" href="www.intechpost.ir">
    <link href="www.intechpost.ir" hreflang="fa" rel="alternate">
    <meta property="og:title" content="شبکه اجتماعی اینتک پست">
    <meta property="og:url" content="www.intechpost.com">
    <meta property="og:type" content="website">
    <meta property="og:description" content="شبکه اجتماعی اینتک پست یک شبکه اجتماعی مبتنی بر متن است که در آن می توان پست هایی تا حداکثر هزار کاراکتر و به همراه یک فایل صوتی، تصویری یت ویدیویی در هشت دسته بندی مختلف گذاشت.">
...
';
        ...
        while($repostrow2 = $result->fetch_assoc()) {

            $uname3 = $repostrow2['username'];
            $repost2 = $repostrow2['repost'];
            $postid2 = $repostrow2['postid'];
        ...
        while($repostrow = $result->fetch_assoc()) {

                
                echo '<br>
       <div class="post" dir="ltr" style="text-align: left">
       <img src="uploads/'.$repostrow['profile'].'" class="profile">';

            if ($_SESSION['username'] === $repostrow['username']) {
            } else {
                $sql3 = "SELECT * FROM follower WHERE follower='".$_SESSION['username']."' && followed='".$repostrow['username']."'";
            $result3 = $conn->query($sql3);
            if ($result3->num_rows>0) {
                echo '<form action="unfollow.php?follower='.$_SESSION['username'].'&followed='.$repostrow['username'].'" method="post"><button type="submit" class="unfollow"><p style="direction: rtl;color:blue">دنبال شده</p></button></form>';
            } else {
                echo '<form action="follow.php?follower='.$_SESSION['username'].'&followed='.$repostrow['username'].'" method="post"><button type="submit" class="follow">دنبال کردن+</button></form>';
            }
        }
                echo'
            <a href="profile.php?username='.$repostrow['username'].'&username2='.$_SESSION['username'].'" style="text-decoration:none;color:#0088ff">@'.$repostrow['username'].'</a>
            <p dir="rtl">'.$repostrow['name'].'این پست را ریپست کرده است:</p>
       <div style="border:2px solid black;border-radius:10px;padding: 2% 2% 2% 2%">
            ...
            if ($row['format'] == 'jpg' or $row['format'] == 'png' or $row['format'] == 'jpeg' or $row['format'] == 'gif') 
            {
            echo'<center><img src="uploads/'.$row['file'].'" class="file"></center>';
            } elseif  ($row['format'] == 'mp3' or $row['format'] == 'm4a') {
                echo'<center><audio class="audio" controls autoplay muted>
                <source src="uploads/'.$row['file'].'" type="audio/mpeg">
              </audio></center>';
            } elseif ($row['format'] == "mp4" or $row['format'] == "m4v") {
                echo'<center><video width="80%" height="auto" controls>
                <source src="uploads/'.$row['file'].'" type="video/mp4">
              </video></center>';
            }
            echo'<p style="color:#7c7c7c">'.$row['category'].'</p>
            <p style="color:#7c7c7c">'.$row['date'].'</p>';
            $postid = $row['id'];
            $like = $row['numlike'];
            $sql2 = "SELECT * FROM `num-like` WHERE username='".$_SESSION['username']."' && postid='$postid'";
